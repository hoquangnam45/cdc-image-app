...
public class ImageEventHandler {
    ...
    public void handleImageUploadedEvent(String payloadString, @Header(GcpPubSubHeaders.ORIGINAL_MESSAGE) BasicAcknowledgeablePubsubMessage originalMessage) {
        try {
            ...
            // Store user metadata
            UUID userId = UUID.fromString(uploadFileBlob.getMetadata().get(CommonConstant.USER_ID_METADATA));
            String fileName = uploadFileBlob.getMetadata().get(CommonConstant.FILE_NAME_METADATA);
            UUID userImageId = UUID.fromString(uploadFileBlob.getMetadata().get(CommonConstant.FILE_ID_METADATA));
            ...
            if (!hasFileBeenUploaded) {
                ProcessingImage processingImage = loadingUploadedImage(uploadFileBlob);
                if (processingImage == null) {
                    originalMessage.ack();
                    storageClient.delete(uploadFileBlob.getBlobId());
                    return;
                }
                UUID newUploadedImageId = UUID.randomUUID();
                ...
                BlobInfo destBlobInfo = BlobInfo.newBuilder(bucketName, newDestObjectName)
                        ...
                        .setMetadata(Map.of(
                                ...
                                CommonConstant.SIZE_METADATA, processingImage.fileSize().toString(),
                ...
                imageRepository.updateUserImageUploadedImageId(userImageId, uploadedImageId);
            }
            ...
            if (!unprocessedJobConfigurations.isEmpty()) {
                ...
                ProcessingImage processingImage = loadingUploadedImage(uploadedFileBlob);
                ...
                for (ProcessJobConfigurationMdl unprocessJobConfiguration : unprocessedJobConfigurations) {
                    UUID jobId = null;
                    try {
                        UUID thumbnailImageId = UUID.randomUUID();
                        ...
                        String destThumbnailName = "uploaded/" + uploadedImageId + "/thumbnails/" + unprocessJobConfiguration.getId() + "/" + thumbnailImageId;
                        BlobInfo destThumbBlobInfo = BlobInfo.newBuilder(bucketName, destThumbnailName)
                                ...
                                .setMetadata(Map.of(
                                        ...
                                        CommonConstant.SIZE_METADATA, thumbnailProcessingImage.fileSize().toString(),
                                        CommonConstant.MIMETYPE_METADATA, thumbnailProcessingImage.fileType(),
                                        CommonConstant.FILE_ID_METADATA, thumbnailImageId.toString(),
                                        CommonConstant.OUTPUT_FORMAT_METADATA, unprocessJobConfiguration.getOutputFileType(),
                                        CommonConstant.CONFIGURATION_ID_METADATA, unprocessJobConfiguration.getId().toString()
                                )).build();
                        try (WriteChannel writer = storageClient.writer(destThumbBlobInfo);
                             OutputStream outputStream = Channels.newOutputStream(writer)) {
                            ImageIO.write(thumbnailProcessingImage.bufferedImage(), thumbnailProcessingImage.ext(), outputStream);
                            createNewGeneratedImage(destThumbBlobInfo.getBlobId());
                        }
                        imageRepository.updateProcessingJob(jobId, null, JobStatus.COMPLETED);
                    } catch (Throwable e) {
                        if (jobId != null) {
                            imageRepository.updateProcessingJob(jobId, e.getMessage(), JobStatus.FAILED);
                        }
                    }
                }
            }
    ...
    private void createNewGeneratedImage(BlobId blobId) {
        ...
        String filePath = "gs://" + blob.getBucket() + "/" + blob.getName();
        String fileType = metadata.get(CommonConstant.MIMETYPE_METADATA);
        GeneratedImageMdl generatedImageMdl = new GeneratedImageMdl(fileId, originalFileId, configurationId, width, height, blob.getSize().intValue(), filePath, fileType, blob.getMd5(), blob.getCreateTimeOffsetDateTime().toInstant());
    ...
    private ProcessingImage processConfiguration(ProcessJobConfigurationMdl processJobConfiguration, ProcessingImage processingImage) {
        ...
        if (newHeight != null && newWidth == null && processJobConfiguration.getKeepRatio()) {
            newWidth = BigDecimal.valueOf(newHeight).divide(BigDecimal.valueOf(processingImage.height()), RoundingMode.HALF_EVEN).multiply(BigDecimal.valueOf(processingImage.width())).intValue();
        } else if (newHeight == null && newWidth != null && processJobConfiguration.getKeepRatio()) {
            newHeight = BigDecimal.valueOf(newWidth).divide(BigDecimal.valueOf(processingImage.width()), RoundingMode.HALF_EVEN).multiply(BigDecimal.valueOf(processingImage.height())).intValue();
    ...
    private ProcessingImage loadingUploadedImage(Blob blob) throws IOException {
        String mimeType;
        int width;
        int height;
